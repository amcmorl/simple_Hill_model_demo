...
SDE = series dampening element
q   = activation, 0 < q <= 1
q0  = initial activation = 0.001

'''
minmax = lambda x, l, u: max(min(x, u), l)
...
class Muscle:
  def __init__(self, q):
    ...
    self.q_CE_init  = minmax(q, 0.001, 1)   # [] initial muscle activity 0...1
  ...
  def step(self, q, dt):
    ...
    #a_mtc = f_mtc / 1000.0
    f_ce = self.f_ce(self.l_CE, q)
    f_isom = self.f_isom(self.l_CE)
    l_see = self.l_MTC - self.l_CE

    self.v_CE = v_CE
    self.l_CE += self.v_CE*dt

    #return str(self.l_MTC) + " " + str(self.v_MTC) + " " + str(self.v_CE) + " " + str(self.l_CE) + " " + str(f_mtc) + " " + str(a_mtc)
    return f_mtc, f_ce, f_isom, f_see, l_see
